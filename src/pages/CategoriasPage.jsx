...
export default function CategoriasPage() {
  ...
  const cargarDatos = async () => {
    ...
    try {
      ...
      setAviso({ open: true, mensaje: 'No se pudieron cargar las categorías.', tipo: 'error' });
    } finally {
      setCargando(false);
    }
  };
  ...
  const handleGuardar = async (datos) => {
    try {
      if (categoriaEditar) {
        await categoriaService.update(categoriaEditar.idCategoria, datos);
        setAviso({ open: true, mensaje: 'Categoría actualizada.', tipo: 'success' });
      } else {
        await categoriaService.create(datos);
        setAviso({ open: true, mensaje: 'Categoría creada.', tipo: 'success' });
      }
      setFormAbierto(false);
      setCategoriaEditar(null);
      cargarDatos();
    } catch {
      setAviso({ open: true, mensaje: 'No se pudo guardar la categoría.', tipo: 'error' });
    }
  };
  ...
  const handleEliminar = async () => {
    try {
      ...
      setAviso({ open: true, mensaje: 'Categoría eliminada.', tipo: 'success' });
      setCategoriaEliminar(null);
      cargarDatos();
    } catch {
      setAviso({ open: true, mensaje: 'No se pudo eliminar la categoría.', tipo: 'error' });
    }
  };
                ...
                <TableRow key={c.idCategoria} hover>
                  ...
                  <TableCell align="center">
                    <Chip
                      ...
                      variant={c.estado ? 'filled' : 'outlined'}
                    />
                  </TableCell>
                  <TableCell align="right">
                    ...
                    <IconButton size="small" color="error" onClick={() => setCategoriaEliminar(c)}>
